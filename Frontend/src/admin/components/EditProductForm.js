...
const EditProductForm = () => {
  ...
  useEffect(() => {
    if (products?.product && products.product._id === productId && !loaded) {
      ...
      const rawVideos = Array.isArray(p.videoUrls) ? p.videoUrls : [];
      const normalizedVideos = rawVideos.length > 0
        ? rawVideos.map((v) => ({
            videoUrl: v.videoUrl || '',
            publicId: v.publicId || '',
            color: v.color || 'yellow-gold',
          }))
        : (p.videoUrl ? [{ videoUrl: p.videoUrl, publicId: p.videoPublicId || '', color: 'yellow-gold' }] : []);
      ...
      setProductData({
        ...
        videoUrl: normalizedVideos.find((v) => v.color === 'yellow-gold')?.videoUrl || normalizedVideos[0]?.videoUrl || p.videoUrl || '',
        videoPublicId: normalizedVideos.find((v) => v.color === 'yellow-gold')?.publicId || normalizedVideos[0]?.publicId || p.videoPublicId || '',
        ...
        ringSize: p.ringSize || '',
        topWidth: p.topWidth || '',
        topThickness: p.topThickness || '',
        shankWidth: p.shankWidth || '',
        shankThickness: p.shankThickness || '',
